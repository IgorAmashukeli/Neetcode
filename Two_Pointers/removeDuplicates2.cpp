...
idea: the same as in remove duplicates, but if there are two elements
(next in the group is the same as the first), then you copy two elements
...
int removeDuplicates(vector<int> &nums) {
  ...
  while (j != nums.size()) {
    ...
    if (nums[j] != previous || j == 0) {
      nums[i] = nums[j];
      ++i;
      if (i < nums.size() && j + 1 < nums.size() && nums[j + 1] == nums[j]) {
        nums[i] = nums[j + 1];
        ++i;
        ++j;
      }
