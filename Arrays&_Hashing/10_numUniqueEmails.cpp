...
string convert(const std::string &email) {
  // find local name
  std::string local_name = email.substr(0, email.find('@'));

  // find domain name
  std::string domain_name = email.substr(email.find('@'), email.size());

  // if there is '+', delete everything after first '+'
  if (local_name.find('+') != local_name.npos) {
    local_name = local_name.substr(0, local_name.find_first_of('+'));
  }

  // erase + remove all '.' in local_name
  local_name.erase(remove(local_name.begin(), local_name.end(), '.'),
                   local_name.end());

  // restore email name
  std::string name = local_name + '@' + domain_name;
...
int numUniqueEmails(vector<string> &emails) {
  ...
  for (size_t i = 0; i < emails.size(); ++i) {
    unique_emails.insert(convert(emails[i]));
  }
