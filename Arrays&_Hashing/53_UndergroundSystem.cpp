...
"Waterloo" in 38-24 = 14 undergroundSystem.getAverageTime("Leyton", "Waterloo");
...
idea: two hashTables to store information about the checkin and average time
time: O(1)
space: O(n)
...
public:
  ...
  void checkIn(int id, string stationName, int t) {
    // remember check in information for a person
    check_in_info_[id] = {stationName, t};
  }

  void checkOut(int id, string stationName, int t) {
    // find start station for id
    string startStation = check_in_info_[id].first;

    // find start time for id
    int startTime = check_in_info_[id].second;

    // string to save start-end station road info
    string startEnd = startStation + "#" + stationName;

    // time of this user road
    int time = t - startTime;

    // sum of all roads times
    u_int64_t sum = 0;

    // number of roads
    int count = 0;

    // if sum of all roads and number of roads exist, take it
    if (average_times_.contains(startEnd)) {
      sum = average_times_[startEnd].first;
      count = average_times_[startEnd].second;
    }

    // update sum and count in hashTable
    average_times_[startEnd] = {sum + time, count + 1};
  }

  double getAverageTime(string startStation, string endStation) {
    // startEnd station key
    string startEnd = startStation + "#" + endStation;

    // convert sum to double
    double sumTime = static_cast<double>(average_times_[startEnd].first);

    // convert count to double
    double count = static_cast<double>(average_times_[startEnd].second);
...
private:
  unordered_map<int, pair<string, int>> check_in_info_;
  unordered_map<string, pair<u_int64_t, int>> average_times_;
};